...
/**
 * This interface contains all the methods that the animator.util.AnimationFileReader
 * class calls as it reads a file containing the animation and builds a model. It is
...
public interface TweenModelBuilder<T> {
  ...
  /**
   * Overloaded add oval method. Adds a new oval to the model with the
   * given specifications.
   *
   * @param name        the unique name given to this shape
   * @param layer       the layer the oval is in
   * @param cx          the x-coordinate of the center of the oval
   * @param cy          the y-coordinate of the center of the oval
   * @param xRadius     the x-radius of the oval
   * @param yRadius     the y-radius of the oval
  ...
  /**
   * Overloaded add rectangle method. Add a new rectangle to the model with
   ...
   *
   * @param name        the unique name given to this shape
   * @param layer       the layer the rectangle is in
   * @param lx          the x-coordinate of the lower left corner of the
   *                    rectangle
   * @param ly          the y-coordinate of the lower left corner of the
   *                    rectangle
  ...
  /**
   * Rotates the specified shape round it's center from
   *
   * @param name        name of shape to be changed
   * @param fromRot     old shape's rotation in degrees
   * @param toRot       new shape's rotation in degrees
   * @param startTime   tick at which the rotation begins
   * @param endTime     tick at which the rotation ends
   */
  TweenModelBuilder<T> addRotateChange(String name, int fromRot, int toRot, int startTime,
                                       int endTime);

  /**
   * Adds a layer to the model with the given layer order. Lower layer order value is closer
   * to the front of the view. Ex: order = 0 is the front-most layer.
   *
   * @param name    layer name
   * @param order   layer order
   */
  TweenModelBuilder<T> addLayer(String name, int order);
